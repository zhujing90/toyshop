...
class cherry_parallax {
  ...
  function video_parallax_shortcode( $atts, $content = null ) {
    ...
    $result .= '<div class="parallax-content">' . do_shortcode( $content ) . '<div class="clear"></div></div>';
    $result .= '<div class="parallax-bg" data-parallax-type="video" data-img-url="'. $sourcesUrlList['poster'] .'" data-speed="' . $speed . '" data-invert="' . $invert . '" >';
      $result .= '<video class="parallax_media parallax-bg-inner" poster="' .$sourcesUrlList['poster']. '" autoplay loop>';
